...
contract Airbnb {
  ...
  function rentOutproperty(string memory name, string memory description, uint256 price) public {
    ...
    properties[propertyId]= Property(propertyId,price,msg.sender,name,description,true,block.timestamp);
  ...
  function rentProperty(uint256 _propertyId, uint256 checkInDate, uint256 checkoutDate) public payable {
    ...
    require(property.isActive==true,"Not Active Property for rent");

    // Assert that property is available for the dates
    require(property.date>=checkInDate && property.date<= checkoutDate ,"property is not available for the dates");
    // Check the customer has sent an amount equal to (pricePerDay * numberOfDays)
    uint numberOfDays= (checkoutDate-checkInDate)/(24*60*60);
    require(msg.value==(property.price * numberOfDays),"Amount is not Suitable");
  ...
  function _createBooking(uint256 _propertyId, uint256 checkInDate, uint256 checkoutDate) internal {
    ...
    // persist to storage
    bookings[bookingId]=Booking(bookingId,_propertyId,checkInDate,checkoutDate);
    // Retrieve `property` object from the storage
    Property storage property= properties[_propertyId]; 

    // Mark the property booked on the requested dates
    markPropertyAsInactive(property.id);
